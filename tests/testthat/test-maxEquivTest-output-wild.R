test_that("Output of maxEquivTest with type = Wild",{
  ...
  reject_null <- max(abs(maxEquivTest_results$placebo_coefficients)) < maxEquivTest_results$bootstrap_critical_value
  expect_equal(maxEquivTest_results$reject_null_hypothesis, reject_null)
  
  maxEquivTest_results2 <- maxEquivTest(Y = 1, ID= 2, G = 4, 
                                        period = 3, X=c(5,6), 
                                        equiv_threshold = 1, data = sim_data,
                                        pretreatment_period = pre_treatment_period, 
                                        base_period = base_period,
                                        alpha = alpha, B=B, type = "Wild")
